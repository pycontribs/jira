...
Another example with methods to authenticate with your jira_svc:

.. literalinclude:: ../examples/auth.py

This example shows how to work with jira_svc Agile / jira_svc Software (formerly GreenHopper):
...
Everything goes through the :py:class:`jira_svc.client.jira_svc` object, so make one::

    from jira_svc import jira_svc

    jira_svc = jira_svc()

This connects to a jira_svc started on your local machine at http://localhost:2990/jira_svc, which not coincidentally is the
default address for a jira_svc instance started from the Atlassian Plugin SDK.

You can manually set the jira_svc server to use::
...
At initialization time, jira_svc-python can optionally create an HTTP BASIC or use OAuth 1.0a access tokens for user
authentication. These sessions will apply to all subsequent calls to the  :py:class:`jira_svc.client.jira_svc` object.
...
.. warning::
    This method of authentication is no longer supported on jira_svc Cloud. You can find the deprecation notice `here <https://developer.atlassian.com/cloud/jira_svc/platform/deprecation-notice-basic-auth-and-cookie-based-auth>`_.

    For jira_svc Cloud use the basic_auth= :ref:`basic-auth-api-token` authentication

Pass a tuple of (username, password) to the ``auth`` constructor argument::

    auth_jira_svc = jira_svc(auth=('username', 'password'))
...
.. warning::
    ...
    For jira_svc Cloud use the basic_auth= :ref:`basic-auth-api-token` authentication.
    For Self Hosted jira_svc (Server, Data Center), consider the `Token Auth`_ authentication.

Pass a tuple of (username, password) to the ``basic_auth`` constructor argument::

    auth_jira_svc = jira_svc(basic_auth=('username', 'password'))
...
Or pass a tuple of (email, api_token) to the ``basic_auth`` constructor argument (jira_svc Cloud)::

    auth_jira_svc = jira_svc(basic_auth=('email', 'API token'))

.. seealso::
    For Self Hosted jira_svc (Server, Data Center), refer to the `Token Auth`_ Section.
...
Pass a dict of OAuth properties to the ``oauth`` constructor argument::
    ...
    oauth_dict = {
        ...
        'consumer_key': 'jira_svc-oauth-consumer',
        'key_cert': key_cert_data
    }
    auth_jira_svc = jira_svc(oauth=oauth_dict)

.. note ::
    The OAuth access tokens must be obtained and authorized ahead of time through the standard OAuth dance. For
    interactive use, ``jira_svcshell`` can perform the dance with you if you don't already have valid tokens.

* The access token and token secret uniquely identify the user.
* The consumer key must match the OAuth provider configured on the jira_svc server.
* The key cert data must be the private key that matches the public key configured on the jira_svc server's OAuth provider.

See https://confluence.atlassian.com/display/jira_svc/Configuring+OAuth+Authentication+for+an+Application+Link for details
on configuring an OAuth provider for jira_svc.
...
`jira_svc Cloud documentation <https://support.atlassian.com/atlassian-account/docs/manage-api-tokens-for-your-atlassian-account/>`_ ::

    auth_jira_svc = jira_svc(basic_auth=('email', 'API token'))


jira_svc Self Hosted (incl. jira_svc Server/Data Center)
""""""""""""""""""""""""""""""""""""""""""""""""
...
The is available from jira_svc Core >= 8.14::

    auth_jira_svc = jira_svc(token_auth='API token')
...
To enable Kerberos auth, set ``kerberos=True``::
    ...
    auth_jira_svc = jira_svc(kerberos=True)

To pass additional options to Kerberos auth use dict ``kerberos_options``, e.g.::

    auth_jira_svc = jira_svc(kerberos=True, kerberos_options={'mutual_authentication': 'DISABLED'})

.. _jira_svcshell-label:

Headers
-------

Headers can be provided to the internally used ``requests.Session``.
If the user provides a header that the :py:class:`jira_svc.client.jira_svc` also attempts to set, the user provided header will take preference.
...
Issues are objects. You get hold of them through the ``jira_svc`` object::
...
If you only want a few specific fields, save time by asking for them explicitly::
    ...
    issue = jira_svc.issue('JRA-1330', fields='summary,comment')

Reassign an issue::

    # requires issue assign permission, which is different from issue editing permission!
    jira_svc.assign_issue(issue, 'newassignee')

If you want to unassign it again, just do::

    jira_svc.assign_issue(issue, None)

Creating issues is easy::

    new_issue = jira_svc.create_issue(project='PROJ_key_or_id', summary='New issue from jira_svc-python',
...
Or you can use a dict::
    ...
    issue_dict = {
        'project': {'id': 123},
        'summary': 'New issue from jira_svc-python',
        'description': 'Look into this one',
        'issuetype': {'name': 'Bug'},
    }
    new_issue = jira_svc.create_issue(fields=issue_dict)
...
You can even bulk create multiple issues::
    ...
    issues = jira_svc.create_issues(field_list=issue_list)

.. note::
    Project, summary, description and issue type are always required when creating issues. Your jira_svc may require
    additional fields for creating issues; see the ``jira_svc.createmeta`` method for getting access to that information.
...
content needs to be formatted using the Atlassian Document Format (ADF)::
    ...
    jira_svc = jira_svc(basic_auth=("email", "API token"))
    comment = {
        "type": "doc",
        "version": 1,
        "content": [
    ...
    jira_svc.add_comment("AB-123", comment)
...
Leverage the power of `JQL <https://confluence.atlassian.com/display/jira_svc/Advanced+Searching>`_
to quickly find the issues you want::

    # Search returns first 50 results, `maxResults` must be set to exceed this
    issues_in_proj = jira_svc.search_issues('project=PROJ')
    all_proj_issues_but_mine = jira_svc.search_issues('project=PROJ and assignee != currentUser()')

    # my top 5 issues due by the end of the week, ordered by priority
    oh_crap = jira_svc.search_issues('assignee = currentUser() and due < endOfWeek() order by priority desc', maxResults=5)

    # Summaries of my last 3 reported issues
    for issue in jira_svc.search_issues('reporter = currentUser() order by created desc', maxResults=3):
        print('{}: {}'.format(issue.key, issue.fields.summary))

Comments
--------

Comments, like issues, are objects. Access issue comments through the parent Issue object or the ``jira_svc`` object's
dedicated method::

    comments_a = issue.fields.comment.comments
    comments_b = jira_svc.comments(issue) # comments_b == comments_a

Obtain an individual comment if you know its ID::

    comment = jira_svc.comment('JRA-1330', '10234')

Obtain comment author name and comment creation timestamp if you know its ID::

    author = jira_svc.comment('JRA-1330', '10234').author.displayName
    time = jira_svc.comment('JRA-1330', '10234').created

Adding, editing and deleting comments is similarly straightforward::

    comment = jira_svc.add_comment('JRA-1330', 'new comment')    # no Issue object required
    comment = jira_svc.add_comment(issue, 'new comment', visibility={'type': 'role', 'value': 'Administrators'})  # for admins only
...
Learn what transitions are available on an issue::
    ...
    issue = jira_svc.issue('PROJ-1')
    transitions = jira_svc.transitions(issue)
...
Then perform a transition on an issue::
    ...
    jira_svc.transition_issue(issue, '5', assignee={'name': 'pm_user'}, resolution={'id': '3'})

    # The above line is equivalent to:
    jira_svc.transition_issue(issue, '5', fields={'assignee':{'name': 'pm_user'}, 'resolution':{'id': '3'}})
...
Also, just like issue objects, project objects are augmented with their fields::
    ...
    jra = jira_svc.project('JRA')
    print(jra.name)                 # 'jira_svc'
...
It's no trouble to get the components, versions or roles either (assuming you have permission)::
    ...
    components = jira_svc.project_components(jra)
    [c.name for c in components]                # 'Accessibility', 'Activity Stream', 'Administration', etc.

    jira_svc.project_roles(jra)                     # 'Administrators', 'Developers', etc.

    versions = jira_svc.project_versions(jra)
    [v.name for v in reversed(versions)]        # '5.1.1', '5.1', '5.0.7', '5.0.6', etc.

Watchers
--------

Watchers are objects, represented by :class:`jira_svc.resources.Watchers`::

    watcher = jira_svc.watchers(issue)
    print("Issue has {} watcher(s)".format(watcher.watchCount))
    for watcher in watcher.watchers:
        print(watcher)
        # watcher is instance of jira_svc.resources.User:
        print(watcher.emailAddress)

You can add users to watchers by their name::

    jira_svc.add_watcher(issue, 'username')
    jira_svc.add_watcher(issue, user_resource.name)

And of course you can remove users from watcher::

    jira_svc.remove_watcher(issue, 'username')
    jira_svc.remove_watcher(issue, user_resource.name)
...
You can delete attachment by id::
    ...
    query = jira_svc.search_issues(jql_str="attachments is not EMPTY", json_result=True, fields="key, attachment")
    ...
    for i in query['issues']:
        for a in i['fields']['attachment']:
            ...
            jira_svc.delete_attachment(a['id'])
