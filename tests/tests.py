...
test_* : For other tests of the non-resource elements of the jira_svc package.
"""
...
from jira_svc import jira_svc, Issue, jira_svcError
from jira_svc.client import ResultList
from jira_svc.resources import Dashboard, Resource, cls_for_resource
from tests.conftest import jira_svcTestCase, allow_on_cloud, rndpassword

LOGGER = logging.getLogger(__name__)


class UniversalResourceTests(jira_svcTestCase):
    def test_universal_find_existing_resource(self):
        resource = self.jira_svc.find("issue/{0}", self.test_manager.project_b_issue1)
        issue = self.jira_svc.issue(self.test_manager.project_b_issue1)
    ...
    def test_find_invalid_resource_raises_exception(self):
        with self.assertRaises(jira_svcError) as cm:
            self.jira_svc.find("woopsydoodle/{0}", "666")
    ...
    def test_pickling_resource(self):
        ...
        unpickled_instance = cls(
            self.jira_svc._options, self.jira_svc._session, raw=pickle.loads(pickled)
        )
    ...
    def test_hashable(self):
        resource = self.jira_svc.find("issue/{0}", self.test_manager.project_b_issue1)
        resource2 = self.jira_svc.find("issue/{0}", self.test_manager.project_b_issue2)
...
class ApplicationPropertiesTests(jira_svcTestCase):
    def test_application_properties(self):
        props = self.jira_svc.application_properties()
        for p in props:
    ...
    def test_application_property(self):
        clone_prefix = self.jira_svc.application_properties(
            key="jira_svc.lf.text.headingcolour"
        )
        self.assertEqual(clone_prefix["value"], "#172b4d")

    def test_set_application_property(self):
        prop = "jira_svc.lf.favicon.hires.url"
        valid_value = "/jira_svc-favicon-hires.png"
        invalid_value = "/invalid-jira_svc-favicon-hires.png"

        self.jira_svc.set_application_property(prop, invalid_value)
        self.assertEqual(
            self.jira_svc.application_properties(key=prop)["value"], invalid_value
        )
        self.jira_svc.set_application_property(prop, valid_value)
        self.assertEqual(
            self.jira_svc.application_properties(key=prop)["value"], valid_value
        )

    def test_setting_bad_property_raises(self):
        prop = "random.nonexistent.property"
        self.assertRaises(jira_svcError, self.jira_svc.set_application_property, prop, "666")


class FieldsTests(jira_svcTestCase):
    def test_fields(self):
        fields = self.jira_svc.fields()
        self.assertGreater(len(fields), 10)


class MyPermissionsServerTests(jira_svcTestCase):
    def setUp(self):
        super().setUp()
        self.issue_1 = self.test_manager.project_b_issue1

    def test_my_permissions(self):
        perms = self.jira_svc.my_permissions()
        self.assertGreaterEqual(len(perms["permissions"]), 40)

    def test_my_permissions_by_project(self):
        perms = self.jira_svc.my_permissions(projectKey=self.test_manager.project_a)
        self.assertGreaterEqual(len(perms["permissions"]), 10)
        perms = self.jira_svc.my_permissions(projectId=self.test_manager.project_a_id)
        self.assertGreaterEqual(len(perms["permissions"]), 10)

    def test_my_permissions_by_issue(self):
        perms = self.jira_svc.my_permissions(issueKey=self.issue_1)
        self.assertGreaterEqual(len(perms["permissions"]), 10)
        perms = self.jira_svc.my_permissions(
...
@allow_on_cloud
class MyPermissionsCloudTests(jira_svcTestCase):
    def setUp(self):
        super().setUp()
        if not self.jira_svc._is_cloud:
            self.skipTest("cloud only test class")
        self.issue_1 = self.test_manager.project_b_issue1
        self.permission_keys = "BROWSE_PROJECTS,CREATE_ISSUES,ADMINISTER_PROJECTS"

    def test_my_permissions(self):
        perms = self.jira_svc.my_permissions(permissions=self.permission_keys)
    ...
    def test_missing_required_param_my_permissions_raises_exception(self):
        with self.assertRaises(jira_svcError):
            self.jira_svc.my_permissions()

    def test_invalid_param_my_permissions_raises_exception(self):
        with self.assertRaises(jira_svcError):
            self.jira_svc.my_permissions("INVALID_PERMISSION")


class SearchTests(jira_svcTestCase):
    def setUp(self):
        jira_svcTestCase.setUp(self)
        self.issue = self.test_manager.project_b_issue1

    def test_search_issues(self):
        issues = self.jira_svc.search_issues(f"project={self.project_b}")
    ...
    def test_search_issues_async(self):
        original_val = self.jira_svc._options["async"]
        try:
            self.jira_svc._options["async"] = True
            ...
            self.jira_svc._options["async"] = original_val

    def test_search_issues_maxresults(self):
        issues = self.jira_svc.search_issues(f"project={self.project_b}", maxResults=10)
    ...
    def test_search_issues_field_limiting(self):
        issues = self.jira_svc.search_issues(f"key={self.issue}", fields="summary,comment")
        issues = cast(ResultList[Issue], issues)
    ...
    def test_search_issues_expand(self):
        issues = self.jira_svc.search_issues(f"key={self.issue}", expand="changelog")
        issues = cast(ResultList[Issue], issues)
...
class ServerInfoTests(jira_svcTestCase):
    def test_server_info(self):
        server_info = self.jira_svc.server_info()
        self.assertIn("baseUrl", server_info)
        self.assertIn("version", server_info)


class OtherTests(jira_svcTestCase):
    def setUp(self) -> None:
        pass  # we don't need jira_svc instance here

    def test_session_invalid_login(self):
        try:
            jira_svc(
                "https://jira_svc.atlassian.com",
                basic_auth=("xxx", "xxx"),
                validate=True,
                logging=False,
            )
        except Exception as e:
            self.assertIsInstance(e, jira_svcError)
            e = cast(jira_svcError, e)  # help mypy
            # 20161010: jira_svc cloud returns 500
            assert e.status_code in (401, 500, 403)
            str(jira_svcError)  # to see that this does not raise an exception
            return
        assert False


class SessionTests(jira_svcTestCase):
    def test_session(self):
        user = self.jira_svc.session()
        self.assertIsNotNone(user.raw["self"])
        self.assertIsNotNone(user.raw["name"])

    def test_session_with_no_logged_in_user_raises(self):
        anon_jira_svc = jira_svc("https://jira_svc.atlassian.com", logging=False)
        self.assertRaises(jira_svcError, anon_jira_svc.session)

    def test_session_server_offline(self):
        try:
            jira_svc("https://127.0.0.1:1", logging=False, max_retries=0)
        except Exception as e:
            self.assertIn(
                type(e),
                (jira_svcError, requests.exceptions.ConnectionError, AttributeError),
                e,
            )
            return
        self.assertTrue(False, "Instantiation of invalid jira_svc instance succeeded.")


MIMICKED_BACKEND_BATCH_SIZE = 10


class AsyncTests(jira_svcTestCase):
    def setUp(self):
    ...
    @parameterized.expand(
        [
            (
                ...
                {Issue: None},
                False,
            ),  # original behaviour, fetch all with jira_svc's original return size
    ...
    def test_fetch_pages(
        ...
        """Tests that the jira_svc._fetch_pages method works as expected."""
        params = {"startAt": 0}
        self.jira_svc._options["default_batch_size"] = default_batch_sizes
        batch_size = self.jira_svc._get_batch_size(Issue)
        expected_calls = _calculate_calls_for_fetch_pages(
            "https://jira_svc.atlassian.com/rest/api/2/search",
            start_at,
            total,
            max_results,
            batch_size,
        ...
        self.jira_svc._session.close()
        self.jira_svc._session = mock_session
        items = self.jira_svc._fetch_pages(
            Issue, "issues", "search", start_at, max_results, params=params
        )

        actual_calls = [[kall[1], kall[2]] for kall in self.jira_svc._session.method_calls]
...
def test_get_batch_size(default_batch_sizes, item_type, expected, no_fields):
    jira_svc = jira_svc(default_batch_sizes=default_batch_sizes, get_server_info=False)

    assert jira_svc._get_batch_size(item_type) == expected
...
def _create_issue_search_results_json(issues, **kwargs):
    """Returns a minimal json object for jira_svc issue search results."""
    return {
...
class ClientRemoteLinkTests(jira_svcTestCase):
    def setUp(self):
        jira_svcTestCase.setUp(self)
        self.issue_key = self.test_manager.project_b_issue1

    def test_delete_remote_link_by_internal_id(self):
        link = self.jira_svc.add_remote_link(
            self.issue_key,
            destination=DEFAULT_NEW_REMOTE_LINK_OBJECT,
        )
        _id = link.id
        self.jira_svc.delete_remote_link(self.issue_key, internal_id=_id)
        self.assertRaises(jira_svcError, self.jira_svc.remote_link, self.issue_key, _id)
    ...
    def test_delete_remote_link_by_global_id(self):
        ...
        _id = link.id
        self.jira_svc.delete_remote_link(
            self.issue_key, global_id="python-test:story.of.sasquatch.riding"
        )
        self.assertRaises(jira_svcError, self.jira_svc.remote_link, self.issue_key, _id)

    def test_delete_remote_link_with_invalid_args(self):
        self.assertRaises(ValueError, self.jira_svc.delete_remote_link, self.issue_key)


class WebsudoTests(jira_svcTestCase):
    def test_kill_websudo(self):
        self.jira_svc.kill_websudo()

    # def test_kill_websudo_without_login_raises(self):
    #    self.assertRaises(ConnectionError, jira_svc)


class UserAdministrationTests(jira_svcTestCase):
    def setUp(self):
        jira_svcTestCase.setUp(self)
    ...
    def _skip_pycontribs_instance(self):
        pytest.skip(
            "The current ci jira_svc admin user for "
    ...
    def _should_skip_for_pycontribs_instance(self):
        # return True
        return self.test_manager.CI_jira_svc_ADMIN == "ci-admin" and (
            self.test_manager.CI_jira_svc_URL == "https://pycontribs.atlassian.net"
        )
    ...
    def test_add_and_remove_user(self):
        ...
        try:
            ...
            self.jira_svc.add_user(
                self.test_username, self.test_email, password=self.test_password
            )
        except jira_svcError:
            pass

        result = self.jira_svc.delete_user(self.test_username)
        assert result, True

        x = -1
        # avoiding a zombie due to Atlassian caching
        for i in range(10):
            x = self.jira_svc.search_users(self.test_username)
            if len(x) == 0:
                break
            sleep(1)
        ...
        result = self.jira_svc.add_user(
            ...
            application_keys=["jira_svc-software"],
        )
        assert result, True
    ...
    def test_add_group(self):
        if self._should_skip_for_pycontribs_instance():
            self._skip_pycontribs_instance()
        try:
            self.jira_svc.remove_group(self.test_groupname)
        except jira_svcError:
            pass

        sleep(2)  # avoid 500 errors
        result = self.jira_svc.add_group(self.test_groupname)
        assert result, True

        x = self.jira_svc.groups(query=self.test_groupname)
        self.assertEqual(
            self.test_groupname,
            x[0],
    ...
    def test_remove_group(self):
        if self._should_skip_for_pycontribs_instance():
            self._skip_pycontribs_instance()
        try:
            self.jira_svc.add_group(self.test_groupname)
            sleep(1)  # avoid 400
        except jira_svcError:
            pass

        result = self.jira_svc.remove_group(self.test_groupname)
        assert result, True
    ...
    def test_add_user_to_group(self):
        try:
            ...
            self.jira_svc.remove_user_from_group(self.test_username, self.test_groupname)
        except jira_svcError:
            pass

        result = self.jira_svc.add_user_to_group(self.test_username, self.test_groupname)
        assert result, True

        x = self.jira_svc.group_members(self.test_groupname)
        self.assertIn(
            self.test_username,
            x.keys(),
    ...
    def test_remove_user_from_group(self):
        ...
        try:
            self.jira_svc.add_user_to_group(self.test_username, self.test_groupname)
        except jira_svcError:
            pass

        result = self.jira_svc.remove_user_from_group(
...
class jira_svcShellTests(jira_svcTestCase):
    def setUp(self) -> None:
        pass  # jira_svc Instance not required

    def test_jira_svcshell_command_exists(self):
        result = os.system("jira_svcshell --help")
