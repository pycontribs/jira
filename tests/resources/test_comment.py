...
class CommentTests(jira_svcTestCase):
    def setUp(self):
    ...
    def tearDown(self) -> None:
        for issue in [self.issue_1_key, self.issue_2_key, self.issue_3_key]:
            for comment in self.jira_svc.comments(issue):
                comment.delete()

    def test_comments(self):
        for issue in [self.issue_1_key, self.jira_svc.issue(self.issue_2_key)]:
            self.jira_svc.issue(issue)
            comment1 = self.jira_svc.add_comment(issue, "First comment")
            comment2 = self.jira_svc.add_comment(issue, "Second comment")
            comments = self.jira_svc.comments(issue)
    ...
    def test_expanded_comments(self):
        comment1 = self.jira_svc.add_comment(self.issue_1_key, "First comment")
        comment2 = self.jira_svc.add_comment(self.issue_1_key, "Second comment")
        comments = self.jira_svc.comments(self.issue_1_key, expand="renderedBody")
        self.assertTrue(hasattr(comments[0], "renderedBody"))
        ret_comment1 = self.jira_svc.comment(
            self.issue_1_key, comment1.id, expand="renderedBody"
        )
        ret_comment2 = self.jira_svc.comment(self.issue_1_key, comment2.id)
        comment1.delete()
        comment2.delete()
        self.assertTrue(hasattr(ret_comment1, "renderedBody"))
        self.assertFalse(hasattr(ret_comment2, "renderedBody"))
        comments = self.jira_svc.comments(self.issue_1_key)
        assert len(comments) == 0

    def test_add_comment(self):
        comment = self.jira_svc.add_comment(
            self.issue_3_key,
            "a test comment!",
    ...
    def test_add_comment_with_issue_obj(self):
        issue = self.jira_svc.issue(self.issue_3_key)
        comment = self.jira_svc.add_comment(
            issue,
            "a new test comment!",
            visibility={"type": "role", "value": "Administrators"},
    ...
    def test_update_comment(self):
        comment = self.jira_svc.add_comment(self.issue_3_key, "updating soon!")
