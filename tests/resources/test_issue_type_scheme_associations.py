...
class IssueTypeSchemeAssociationTests(jira_svcTestCase):
    def test_scheme_associations(self):
        all_schemes = self.jira_svc.issue_type_schemes()
        # there should be more than 1 scheme
        self.assertGreaterEqual(len(all_schemes), 2)
        test_pass = False
        for scheme in all_schemes:
            associations = self.jira_svc.get_issue_type_scheme_associations(scheme["id"])
